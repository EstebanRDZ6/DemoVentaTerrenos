...
class SiteHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ValueListenableBuilder<AppUser?>(
        ...
        builder: (context, user, _) => ValueListenableBuilder<AppLanguage>(
          valueListenable: AppLocale.instance.language,
          builder: (context, language, _) => Material(
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  return Container(
                    ...
                    child: Row(children: [
                      InkWell(borderRadius: BorderRadius.circular(14), onTap: onGoHome, child: Row(mainAxisSize: MainAxisSize.min, children: [Container(width: 42, height: 42, decoration: BoxDecoration(gradient: LinearGradient(colors: [scheme.primary, scheme.primary.withValues(alpha: .72)]), borderRadius: BorderRadius.circular(13)), child: const Icon(Icons.location_on_outlined, color: Colors.white)), if (!compact) ...[const SizedBox(width: 10), Text('Sitios Propiedades', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w850))]])),
                      const Spacer(),
                      if (!compact) ...[
                        _NavItem(label: 'Inicio', selected: currentRoute == AppRoutes.home, onTap: onGoHome),
                        _NavItem(label: 'Venta', selected: currentRoute == AppRoutes.sales, onTap: onGoSales),
                        _NavItem(label: 'Alquiler', selected: currentRoute == AppRoutes.rentals, onTap: onGoRentals),
                        _NavItem(label: 'Mapa', selected: currentRoute == AppRoutes.map, onTap: () => Navigator.pushNamed(context, AppRoutes.map)),
                        _NavItem(label: 'Vendé con nosotros', selected: currentRoute == AppRoutes.sellWithUs, onTap: () => Navigator.pushNamed(context, AppRoutes.sellWithUs)),
                        _NavItem(label: 'Contacto', selected: currentRoute == AppRoutes.contact, onTap: onGoContact),
                        if (user?.canUseCrm == true) _NavItem(label: 'CRM', selected: currentRoute == AppRoutes.crm, onTap: onGoCrm),
                        if (user?.canUseCrm == true) _NavItem(label: 'ERP', selected: currentRoute == AppRoutes.erp, onTap: onGoErp ?? () => Navigator.pushNamed(context, AppRoutes.erp)),
                      ],
