...
class _PropertyListScreenState extends State<PropertyListScreen> {
  ...
  void _resetPriceRange() {
    ...
    _maxPrice = _currencyUsd ? (isSale ? 8000 : 600) : (isSale ? 11200000 : 840000);
  ...
  Widget build(BuildContext context) {
    ...
    final List<PropertyListing> listings = <PropertyListing>[
      ..._service.fetchByType(widget.type),
      ...DemoListingStore.instance.created.value.where((PropertyListing item) => item.type == widget.type),
    ];
    ...
    filtered.sort((PropertyListing a, PropertyListing b) {
      final double aPrice = _currencyUsd ? a.priceUsd : a.priceUsd * _arsPerUsd;
      final double bPrice = _currencyUsd ? b.priceUsd : b.priceUsd * _arsPerUsd;
              ...
              child: Center(
                child: ConstrainedBox(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      Text(
                        isSale ? 'Terrenos en venta' : 'Propiedades en alquiler',
  ...
  bool get _hasActiveFilters {
    final bool isSale = widget.type == ListingType.sale;
    final double defaultMax = _currencyUsd ? (isSale ? 8000 : 600) : (isSale ? 11200000 : 840000);
    return _selectedLocations.isNotEmpty ||
      ...
      child: Padding(
        ...
        child: LayoutBuilder(
          builder: (BuildContext context, BoxConstraints constraints) {
            ...
            final List<Widget> fields = _filterFields(minPrice, maxPrice, isSale);
              ...
              children: <Widget>[
                Row(
                  children: <Widget>[
                    ...
                    Expanded(
                      child: Text('Buscar y filtrar', style: Theme.of(context).textTheme.titleLarge),
                    ),
                    _currencyToggle(),
                  ],
                ),
                ...
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: MockPropertyService.propertyKinds
                      .map(
  ...
  Widget _currencyToggle() {
    return SegmentedButton<bool>(
      segments: const <ButtonSegment<bool>>[
        ButtonSegment<bool>(value: true, label: Text('USD')),
        ButtonSegment<bool>(value: false, label: Text('ARS')),
      ],
      selected: <bool>{_currencyUsd},
      onSelectionChanged: (Set<bool> selection) {
        setState(() {
          _currencyUsd = selection.first;
          _resetPriceRange();
        });
      },
    );
    ...
    return <Widget>[
      ...
      SizedBox(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            Text('Hasta ${_priceLabel(_draftMaxPrice)}', style: Theme.of(context).textTheme.titleMedium),
            Slider(
              value: _draftMaxPrice.clamp(minPrice, maxPrice).toDouble(),
              min: minPrice,
              max: maxPrice,
              divisions: isSale ? 16 : 18,
              label: _priceLabel(_draftMaxPrice),
  ...
  String _priceLabel(double value) {
    return _currencyUsd ? 'USD ${value.toStringAsFixed(0)}' : 'ARS ${value.toStringAsFixed(0)}';
  }
